...
export interface UseFunctionRunsResult {
  ...
  clearSelection: () => void;
  refresh: (overrides?: { limit?: number; pageToken?: string }) => Promise<FunctionRunSummary[]>;
  loadMore: (overrides?: { limit?: number }) => Promise<FunctionRunSummary[]>;
}
...
export function useFunctionRuns({
  ...
  const refresh = useCallback(async (overrides: { limit?: number; pageToken?: string } = {}, signal?: AbortSignal): Promise<FunctionRunSummary[]> => {
    if (!isEnabled) {
      setRuns([]);
      setTotal(0);
  ...
  const loadMore = useCallback(async (overrides: { limit?: number } = {}): Promise<FunctionRunSummary[]> => {
