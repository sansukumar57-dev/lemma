...
from lemma_connectors.jira.generated.tool_types import EvaluateJiraExpressionToolInput, EvaluateJiraExpressionToolOutput
from lemma_connectors.core.resource import BaseResourceClient, coerce_tool_result, operation

class EvaluateJiraExpressionInput(EvaluateJiraExpressionToolInput):
    """Operation input for `evaluate_jira_expression`."""
    pass

class EvaluateJiraExpressionOutput(EvaluateJiraExpressionToolOutput):
    """Operation output for `evaluate_jira_expression`."""
    pass

class JiraEvaluateJiraResource(BaseResourceClient):
    """Operations for the `evaluate_jira` resource."""

    @operation(
        name='evaluate_jira_expression',
        title='EvaluateJiraExpression',
        input_model=EvaluateJiraExpressionInput,
        output_model=EvaluateJiraExpressionOutput,
        tools_used=('evaluate_jira_expression',),
        tags=tuple(['Jira expressions']),
    )
    async def expression(self, data: EvaluateJiraExpressionInput) -> EvaluateJiraExpressionOutput:
...
Important inputs: expand, body"""
        tool = self._client.get_tool('evaluate_jira_expression')
        result = await tool.execute(data.model_dump(exclude_none=True, exclude_unset=True, by_alias=False))
        return EvaluateJiraExpressionOutput.model_validate(coerce_tool_result(result))
