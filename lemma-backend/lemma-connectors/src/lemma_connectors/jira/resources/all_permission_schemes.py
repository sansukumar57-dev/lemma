...
from lemma_connectors.jira.generated.tool_types import GetAllPermissionSchemesToolInput, GetAllPermissionSchemesToolOutput
from lemma_connectors.core.resource import BaseResourceClient, coerce_tool_result, operation

class GetAllPermissionSchemesInput(GetAllPermissionSchemesToolInput):
    """Operation input for `get_all_permission_schemes`."""
    pass

class GetAllPermissionSchemesOutput(GetAllPermissionSchemesToolOutput):
    """Operation output for `get_all_permission_schemes`."""
    pass

class JiraAllPermissionSchemesResource(BaseResourceClient):
    """Operations for the `all_permission_schemes` resource."""

    @operation(
        name='get_all_permission_schemes',
        title='GetAllPermissionSchemes',
        input_model=GetAllPermissionSchemesInput,
        output_model=GetAllPermissionSchemesOutput,
        tools_used=('get_all_permission_schemes',),
        tags=tuple(['Permission schemes']),
    )
    async def get(self, data: GetAllPermissionSchemesInput) -> GetAllPermissionSchemesOutput:
...
Important inputs: expand"""
        tool = self._client.get_tool('get_all_permission_schemes')
        result = await tool.execute(data.model_dump(exclude_none=True, exclude_unset=True, by_alias=False))
        return GetAllPermissionSchemesOutput.model_validate(coerce_tool_result(result))
