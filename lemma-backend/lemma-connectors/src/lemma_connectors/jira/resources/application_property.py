...
from lemma_connectors.jira.generated.tool_types import GetApplicationPropertyToolInput, GetApplicationPropertyToolOutput, SetApplicationPropertyToolInput, SetApplicationPropertyToolOutput
from lemma_connectors.core.resource import BaseResourceClient, coerce_tool_result, operation

class GetApplicationPropertyInput(GetApplicationPropertyToolInput):
    """Operation input for `get_application_property`."""
    pass

class GetApplicationPropertyOutput(GetApplicationPropertyToolOutput):
    """Operation output for `get_application_property`."""
    pass

class SetApplicationPropertyInput(SetApplicationPropertyToolInput):
    """Operation input for `set_application_property`."""
    pass

class SetApplicationPropertyOutput(SetApplicationPropertyToolOutput):
    """Operation output for `set_application_property`."""
    pass

class JiraApplicationPropertyResource(BaseResourceClient):
    """Operations for the `application_property` resource."""

    @operation(
        name='get_application_property',
        title='GetApplicationProperty',
        input_model=GetApplicationPropertyInput,
        output_model=GetApplicationPropertyOutput,
        tools_used=('get_application_property',),
        tags=tuple(['Jira settings']),
    )
    async def get(self, data: GetApplicationPropertyInput) -> GetApplicationPropertyOutput:
...
Important inputs: permission_level, key_filter"""
        tool = self._client.get_tool('get_application_property')
        result = await tool.execute(data.model_dump(exclude_none=True, exclude_unset=True, by_alias=False))
        return GetApplicationPropertyOutput.model_validate(coerce_tool_result(result))

    @operation(
        name='set_application_property',
        title='SetApplicationProperty',
        input_model=SetApplicationPropertyInput,
        output_model=SetApplicationPropertyOutput,
        tools_used=('set_application_property',),
        tags=tuple(['Jira settings']),
    )
    async def set(self, data: SetApplicationPropertyInput) -> SetApplicationPropertyOutput:
...
Important inputs: id, body"""
        tool = self._client.get_tool('set_application_property')
        result = await tool.execute(data.model_dump(exclude_none=True, exclude_unset=True, by_alias=False))
        return SetApplicationPropertyOutput.model_validate(coerce_tool_result(result))
