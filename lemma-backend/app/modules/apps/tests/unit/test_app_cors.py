...
def test_app_subdomain_regex_matches_slug_and_base(monkeypatch):
    monkeypatch.setattr(cors.settings, "app_base_domain", "apps.asur.work")
    monkeypatch.setattr(cors.settings, "cors_origin_regex", None)

    pattern = cors.get_allowed_cors_origin_regex()
    assert pattern is not None
    assert re.fullmatch(pattern, "https://home-abc123.apps.asur.work")
    assert re.fullmatch(pattern, "https://apps.asur.work")
    assert not re.fullmatch(pattern, "https://apps.asur.work.evil.com")
...
def test_combines_with_configured_regex(monkeypatch):
    ...
    assert re.fullmatch(pattern, "https://home-1.apps.asur.work")
