...
from ...models.record_get_response_record_get import RecordGetResponseRecordGet
...
) -> ErrorResponse | RecordGetResponseRecordGet | None:
    if response.status_code == 200:
        response_200 = RecordGetResponseRecordGet.from_dict(response.json())
...
def _build_response(
    *, client: AuthenticatedClient | Client, response: httpx.Response
) -> Response[ErrorResponse | RecordGetResponseRecordGet]:
    return Response(
...
def sync_detailed(
    ...
    *,
    ...
    """Get Record
     ...
     Fetch one record by primary key value (returns the record object, no envelope). The `record_id` path
    segment is the table's primary key value as stored in the table, not necessarily a UUID.
...
def sync(
    ...
    *,
    ...
    Returns:
        ErrorResponse | RecordGetResponseRecordGet
    """
